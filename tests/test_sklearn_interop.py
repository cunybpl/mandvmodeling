...
import numpy as np
from mandvmodeling.core.estimator import MandVCurvefitEstimator
...
def test_curvefit_estimator_against_scipy_example():
    ...
    estimator = MandVCurvefitEstimator(model_func=func)
...
def test_curvefit_grid_search():
    ...
    search = GridSearchCV(MandVCurvefitEstimator(), param_grid=params)
...
def test_callable_bounds():
    ...
    est = MandVCurvefitEstimator(model_func=f, bounds=bounds)
...
def test_X_and_y_are_accessible_on_estimator_after_fit():
    ...
    est = MandVCurvefitEstimator(model_func=f)
    est.fit(xdata.reshape(-1, 1), ydata)
