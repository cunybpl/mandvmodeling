...
def _validate_n_by_one_dim_timestamp(v: Any) -> NByOneNDArray[np.datetime64]:
  ...
  assert len(arr.shape) == 1, f"Shape of data should be M x 1, got {arr.ndim}-dimensional array."
...
TimestampArrayField = Annotated[
    ...
    WithJsonSchema(
        {"type": "array", "items": {"type": "np.datetime64"}}
    )
    ]
...
class EnergyOATDataModel(CurvefitEstimatorDataModel):
  ...
  def validate_all(self) -> "EnergyOATDataModel":
    ...
    """
    assert len(self.X) == len(self.y) == len(self.sensor_reading_timestamps), "X, y, and sensor_reading_timestamps len must be the same"
