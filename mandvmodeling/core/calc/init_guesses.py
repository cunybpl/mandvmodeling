...
def threepc(
    ...
    """
    X = X.squeeze()
    return (y[0], ((max(y) - min(y)) / (X[-1] - np.median(X))).item(), np.median(X))
...
def threeph(
    ...
    """
    X = X.squeeze()
    return (y[-1], ((min(y) - max(y)) / (np.median(X) - X[0])).item(), np.median(X))
...
def fourp(
    ...
    return (
        min(y),
        ((y[0] - min(y)) / (X[0] - X[y == min(y)])).item(),
        ((y[-1] - min(y)) / (X[-1] - X[y == min(y)])).item(),
        X[y == min(y)].item(),
    )
